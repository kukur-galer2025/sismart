...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <h2 class="text-lg sm:text-xl font-bold" data-lang="kat.title">Kategori Barang</h2>
        <button x-data @click="$dispatch('open-modal', 'modal-create')" class="btn-primary text-xs sm:text-sm w-full sm:w-auto justify-center"><i class="fas fa-plus"></i> <span data-lang="kat.tambah">Tambah Kategori</span></button>
    </div>
    ...
    <div class="sm:hidden space-y-3">
        ...
        <div class="glass rounded-xl p-4">
            <div class="flex items-start justify-between mb-2">
                ...
                <span class="px-2 py-0.5 rounded-full text-[10px] font-medium text-indigo-700 dark:text-indigo-400 bg-indigo-500/10">{{ $k->barangs_count }} <span data-lang="kat.barang">barang</span></span>
            </div>
            <p class="text-xs mb-3" style="color:var(--text-muted)">{{ $k->deskripsi ?? '' }}<span x-show="!('{{ $k->deskripsi }}')" data-lang="kat.tidak_ada_deskripsi">Tidak ada deskripsi</span></p>
        ...
        @empty
        <div class="glass rounded-xl p-8 text-center" style="color:var(--text-muted)"><i class="fas fa-tags text-3xl mb-2 opacity-30"></i><p class="text-sm" data-lang="kat.belum_ada">Belum ada kategori</p></div>
        @endforelse
    </div>
        ...
        <div class="table-responsive">
            <table class="w-full text-sm text-left">
                <thead class="text-xs uppercase border-b" style="color:var(--text-muted); background:var(--bg-input); border-color:var(--border-color)">
                    <tr><th class="px-6 py-4" data-lang="kat.nama">Nama Kategori</th><th class="px-6 py-4" data-lang="kat.deskripsi">Deskripsi</th><th class="px-6 py-4 text-center" data-lang="kat.jumlah_barang">Jumlah Barang</th><th class="px-6 py-4 text-right" data-lang="common.aksi">Aksi</th></tr>
                </thead>
                <tbody>
                    @forelse($kategoris as $k)
                    <tr class="border-b hover:bg-indigo-500/5 dark:hover:bg-white/5 transition-colors" style="border-color:var(--border-color)">
                        ...
                        <td class="px-6 py-4 text-center"><span class="bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 py-1 px-3 rounded-full text-xs">{{ $k->barangs_count }} <span data-lang="kat.barang">barang</span></span></td>
                    ...
                    @empty
                    <tr><td colspan="4" class="px-6 py-8 text-center" style="color:var(--text-muted)" data-lang="kat.tidak_ada_data">Tidak ada data kategori.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div x-show="show" x-transition.scale class="relative w-full max-w-lg glass rounded-2xl shadow-2xl border border-indigo-500/30 overflow-hidden">
            <div class="px-6 pt-6 pb-4">
                <div class="flex justify-between items-center mb-5">
                    <h3 class="text-lg font-semibold" data-lang="kat.tambah">Tambah Kategori</h3>
                ...
                <form action="{{ route('kategori.store') }}" method="POST" class="space-y-4">@csrf
                    <div>
                        <label class="form-label"><i class="fas fa-tag text-indigo-500"></i> <span data-lang="kat.nama">Nama Kategori</span> <span class="text-rose-500">*</span></label>
                        <input type="text" name="nama" required class="form-input" placeholder="Masukkan nama kategori">
                    </div>
                    <div>
                        <label class="form-label"><i class="fas fa-align-left text-indigo-500"></i> <span data-lang="kat.deskripsi">Deskripsi</span></label>
                        <textarea name="deskripsi" rows="3" class="form-input" placeholder="Deskripsi opsional"></textarea>
                    </div>
                    <div class="flex justify-end gap-3 pt-2">
                        <button type="button" @click="show=false" class="btn-outline" data-lang="common.batal">Batal</button>
                        <button type="submit" class="btn-primary"><i class="fas fa-save"></i> <span data-lang="common.simpan">Simpan</span></button>
                    </div>
                </form>
            </div>
        </div>
...
<div x-data="{ show:false, id:'', nama:'', deskripsi:'' }" x-show="show" @edit-kategori.window="show=true; id=$event.detail.id; nama=$event.detail.nama; deskripsi=$event.detail.deskripsi" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div x-show="show" x-transition.scale class="relative w-full max-w-lg glass rounded-2xl shadow-2xl border border-indigo-500/30 overflow-hidden">
            <div class="px-6 pt-6 pb-4">
                <div class="flex justify-between items-center mb-5">
                    <h3 class="text-lg font-semibold" data-lang="kat.edit">Edit Kategori</h3>
                    <button @click="show=false" style="color:var(--text-muted)" class="hover:opacity-70"><i class="fas fa-times"></i></button>
                </div>
                <form :action="'{{ url('kategori') }}/'+id" method="POST" class="space-y-4">@csrf @method('PUT')
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        <button type="button" @click="show=false" class="btn-outline" data-lang="common.batal">Batal</button>
                        <button type="submit" class="btn-primary"><i class="fas fa-save"></i> <span data-lang="common.update">Update</span></button>
                    </div>
                </form>
            </div>
        </div>
