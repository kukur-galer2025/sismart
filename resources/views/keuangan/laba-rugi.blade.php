    ...
    <div class="space-y-3">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-2">
            <form method="GET" action="{{ route('keuangan.laba-rugi') }}" class="flex items-center gap-1.5 flex-wrap">
                <input type="date" name="dari" value="{{ request('dari', \Carbon\Carbon::now()->startOfMonth()->format('Y-m-d')) }}" class="form-input !w-[120px] !py-1.5 !text-[11px]">
                <span class="text-[10px]" style="color:var(--text-muted)">s/d</span>
                <input type="date" name="sampai" value="{{ request('sampai', \Carbon\Carbon::now()->format('Y-m-d')) }}" class="form-input !w-[120px] !py-1.5 !text-[11px]">
