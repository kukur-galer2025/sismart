...
<div class="space-y-6">
    <div class="space-y-3">
        <form method="GET" action="{{ route('barang-masuk.index') }}" class="flex items-center gap-1.5 flex-wrap">
            ...
            @if(request()->anyFilled(['search','dari','sampai']))<a href="{{ route('barang-masuk.index') }}" class="btn-outline !text-[11px] !py-1.5 !px-2.5"><i class="fas fa-times"></i> <span data-lang="common.reset">Reset</span></a>@endif
        </form>
        <div class="flex items-center gap-1.5 flex-wrap">
            <a href="{{ route('barang-masuk.create') }}" class="btn-success !text-[11px] !py-1.5 !px-2.5"><i class="fas fa-plus"></i> Input Masuk</a>
            <a href="{{ route('export.barang-masuk.pdf', ['dari'=>request('dari',now()->startOfMonth()->format('Y-m-d')),'sampai'=>request('sampai',now()->format('Y-m-d'))]) }}" class="btn-danger !text-[11px] !py-1.5 !px-2.5"><i class="fas fa-file-pdf"></i> PDF</a>
            <a href="{{ route('export.barang-masuk.excel', ['dari'=>request('dari',now()->startOfMonth()->format('Y-m-d')),'sampai'=>request('sampai',now()->format('Y-m-d'))]) }}" class="btn-success !text-[11px] !py-1.5 !px-2.5"><i class="fas fa-file-excel"></i> Excel</a>
        </div>
    </div>

    <div class="sm:hidden space-y-3">
        @forelse($transaksis as $trx)
        <a href="{{ route('barang-masuk.show', $trx->id) }}" class="glass rounded-xl p-4 block">
            <div class="flex items-start justify-between mb-2">
                <div><p class="font-medium text-sm">{{ $trx->barang->nama }}</p><p class="text-xs font-mono text-emerald-600 dark:text-emerald-400">{{ $trx->kode_transaksi }}</p></div>
                <span class="text-xs" style="color:var(--text-muted)">{{ $trx->tanggal->format('d/m/Y') }}</span>
            </div>
            <div class="flex items-center justify-between text-xs">
                <span style="color:var(--text-muted)">{{ $trx->supplier ?? '-' }}</span>
                <span class="text-emerald-600 dark:text-emerald-400 font-bold">+{{ number_format($trx->jumlah) }} &rarr; HPP: Rp {{ number_format($trx->total_harga,0,',','.') }}</span>
            </div>
        </a>
        @empty
        ...
        <div class="table-responsive">
            <table class="w-full text-sm text-left">
                <thead><tr style="background:var(--bg-input)">
                    ...
                    <th class="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider hidden lg:table-cell" style="color:var(--text-muted)">Supplier</th>
                    <th class="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-right" style="color:var(--text-muted)">Jumlah</th>
                    <th class="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-right" style="color:var(--text-muted)">Total Nilai / HPP</th>
                ...
                <tbody>
                    ...
                    <tr class="border-t transition-colors hover:bg-indigo-500/[.03] dark:hover:bg-white/[.03]" style="border-color:var(--border-color)">
                        ...
                        <td class="px-5 py-3 font-mono text-xs text-emerald-600 dark:text-emerald-400">{{ $trx->kode_transaksi }}</td>
                        ...
                        <td class="px-5 py-3 hidden lg:table-cell" style="color:var(--text-secondary)">{{ $trx->supplier ?? '-' }}</td>
                        ...
                        <td class="px-5 py-3 text-center"><a href="{{ route('barang-masuk.show', $trx->id) }}" class="p-1.5 rounded-lg hover:bg-indigo-500/10 text-indigo-500 transition-colors inline-block"><i class="fas fa-eye"></i></a></td>
                    </tr>
                    @empty
